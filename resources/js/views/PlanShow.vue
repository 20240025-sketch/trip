  ...
  <div v-else-if="plan" class="max-w-6xl mx-auto">
    ...
    <div class="mb-8">
      ...
      <div class="flex gap-4 text-sm text-gray-600">
        ...
        <span v-if="plan.is_public">🌐 公開中</span>
      </div>

      <div v-if="plan.is_public" class="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <p class="text-sm text-gray-700">
          <strong>公開URL:</strong> 
          <a :href="`/p/${plan.slug}`" target="_blank" class="text-blue-600 hover:underline ml-2">
            {{ window.location.origin }}/p/{{ plan.slug }}
          </a>
        </p>
      </div>
    </div>
...
const downloadPdf = () => {
  window.open(`/api/pdf/plans/${plan.value.id}/download`, '_blank');
};
